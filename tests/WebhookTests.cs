...
namespace GetStream.Tests
{
    ...
    // the SDK's generate.sh copies it into tests/fixtures/webhooks/ at codegen time.
    // -----------------------------------------------------------------------
    [TestFixture]
    public class WebhookConformanceTests
    {
        ...
        private static IEnumerable<string> HappyDirs()
        {
            ...
            var dirs = Directory.EnumerateDirectories(FixtureRoot)
                .Where(d => Path.GetFileName(d) != "_invalid")
        ...
        [TestCaseSource(nameof(HappyDirs))]
        public void Conformance_Happy(string dir)
        {
            var body = File.ReadAllBytes(Path.Combine(dir, "body.json"));
            var bodyGz = File.ReadAllBytes(Path.Combine(dir, "body.gz"));
            var sqsCompressed = File.ReadAllText(Path.Combine(dir, "sqs_body.txt")).Trim();
            var sqsRaw = File.ReadAllText(Path.Combine(dir, "sqs_body_uncompressed.txt")).Trim();
            var sns = File.ReadAllText(Path.Combine(dir, "sns_notification.txt")).Trim();
            var sig = File.ReadAllText(Path.Combine(dir, "signature.txt")).Trim();

            Assert.That(Webhook.VerifySignature(body, sig, CanonicalTestSecret), Is.True);
        ...
        [Test]
        public void Conformance_TamperedBody()
        {
            var dir = NegDir("tampered_body");
        ...
        [Test]
        public void Conformance_UnknownTypeReturnsUnknownEvent()
        {
            var dir = NegDir("unknown_type");
            if (!Directory.Exists(dir))
            {
                Assert.Ignore("fixtures not present");
            }
            var body = File.ReadAllBytes(Path.Combine(dir, "body.json"));
            var ev = Webhook.ParseEvent(body);
            Assert.That(ev, Is.InstanceOf<Webhook.UnknownEvent>());
            Assert.That(((Webhook.UnknownEvent)ev).Type, Is.EqualTo("totally.made.up"));
        }

        [Test]
        public void Conformance_MissingType()
        {
            var dir = NegDir("missing_type");
            if (!Directory.Exists(dir))
            {
                Assert.Ignore("fixtures not present");
            }
        ...
        [Test]
        public void Conformance_MalformedJson()
        {
            var dir = NegDir("malformed_json");
            if (!Directory.Exists(dir))
            {
                Assert.Ignore("fixtures not present");
            }
        ...
        [Test]
        public void Conformance_EmptyBody()
        {
            var dir = NegDir("empty_body");
            if (!Directory.Exists(dir))
            {
                Assert.Ignore("fixtures not present");
            }
        ...
        [Test]
        public void Conformance_BadCompression()
        {
            var dir = NegDir("bad_compression");
            if (!Directory.Exists(dir))
            {
                Assert.Ignore("fixtures not present");
            }
        ...
        [Test]
        public void Conformance_BadSnsEnvelope()
        {
